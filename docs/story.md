...
shots (a further −22 per 100) — while the 2-for-1 launch window at 25–36
seconds costs almost nothing (~−0.8 per 100), upgrading the sibling
play-by-play study's eFG-proxy finding with a real per-shot price.
...
{{fig:fig5_eb_projection_py|Observed shot-making versus its empirical-Bayes posterior, sized by shot volume. The shrinkage rule made visible: high-volume shooters keep their number, small samples are pulled toward the league mean — and this picture is why the projection beats carrying raw numbers forward.}}
